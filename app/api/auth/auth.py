...
def get_user_by_id(user_id: int, db: Session = Depends(get_db)):
    ...
    return JSONResponse(status_code=status.HTTP_200_OK,
                        content={"user_id": user.user_id, "first_name": user.first_name, "last_name": user.last_name,
                                 "email": user.email,"phone_number": user.phone_number,"status": user.status, "role":user.role},
                        headers=headers)
...
def get_users(db: Session = Depends(get_db)):
    ...
    return [{"user_id": user.user_id, "first_name": user.first_name, "last_name": user.last_name,
             "email": user.email, "phone_number": user.phone_number, "status": user.status, "role":user.role} for user in users]
